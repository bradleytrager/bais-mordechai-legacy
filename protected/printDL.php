...
function ListByParashah($parashah){
  ...
  require '../mysql.php';
	...
	//$parashah =  mysql_real_escape_string($parashah);
...
?>
<?
function printDL($parasha, $ndir){
  $parasha = str_replace("\\","",$parasha);
  //$parasha = str_replace(" ","",$parasha);
  $parasha = chop($parasha);
  ...
  $n = 0;
   ...
   while (($file = $dir->read()) !== false)
	{
		if (strstr($file,$parasha)){
			$filenames[$n] = strstr($file,$parasha);
			$n++;
		}
	}
  if ($filenames != null){
...
switch ($ndir){
	case "mp3/":
	echo '<h2>Shiurim for Parashas '.
		$parasha.
		':</h2>';
		break;
	case "mp3/NC/":
	echo '<h2>New Series for Parashas '.
		$parasha.
		':</h2>';
		break;
}
...
echo '<table>';
  ...
  for ($n = 0; $n < sizeof($filenames); $n++){
	echo '<tr><td>'.
	 str_ireplace(".mp3","",$filenames[$n]).
	 '</td><td>'.
      '<object type="application/x-shockwave-flash" data="dewplayer.swf" width="200" height="20" id="dewplayer" name="dewplayer"> <param name="wmode" value="transparent" /><param name="movie" value="dewplayer.swf" /> <param name="flashvars" value="mp3='.
	  $ndir.
	 $filenames[$n].
	  '&amp;nopointer=1" /> <a href="'.
	  $ndir.
